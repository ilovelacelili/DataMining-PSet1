...
def retrieve_items(logger, base_url, access_token, realmId, page_size=100):
    ...
    while True:
        ...
        query = f'SELECT * FROM Item STARTPOSITION {starting_pos} MAXRESULTS {page_size}'
        ...
        for attempt in range(max_tries):
            try:
                ...
                data = response.json().get('QueryResponse', {}).get('Item', [])
...
def load_data(access_token, *args, **kwargs):
    ...
    logger.info('Starting Items Loader...')
    ...
    df_items = pd.DataFrame(raw_data)
